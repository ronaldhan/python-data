...
if '__main__' == __name__:        
            ...
            for row in rows:
                    ...
                    if row[3] is None or row[3].find('...') != -1 or row[3] == '':
                            continue
                    else:
                            name=row[3].strip()
                            print name
                            ...
                            if len(decodejson['data'])>0:
                                    ...
                                    if iscontain is True:
                                            #第一个满足条件的结果中提取经纬度和地址
                                            address=decodejson['data'][index]['address']
                                            lng=decodejson['data'][index]['location']['lng']
                                            lat=decodejson['data'][index]['location']['lat']
                                            mysqlconn.insert(tableName,name=row[3],tags=row[4],grade=row[5],comment=row[6],PageUrl=row[7],address=address,lng=lng,lat=lat)
                                            mysqlconn.commit()
                                            print "记录(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)已写入数据库" % (row[1],row[2],row[3],row[4],row[5],row[6],row[7],address,lng,lat)
